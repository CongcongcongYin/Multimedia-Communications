...
[ Recovery ] = Recover(damaged_mbs,xs,ys,mbsize);
Y_recovr = zeros(imgx, imgy);

for i=1:mbx
    for j=1:mby
        Y_recovr(mbsize*(i-1)+1:mbsize*i,mbsize*(j-1)+1:mbsize*j)=Recovery{i,j};
    end
end

Recovery  = uint8(Y_recovr);    % convert to uint8 as a grayscale image
figure(3);                      % show recovery image
imshow(Recovery);
%%
fprintf('\nPSNR = %0.4f\n', psnr(mandrill_damaged, mandrill));   % PSNR of damaged image
fprintf('\nPSNR = %0.4f\n', psnr(Recovery , mandrill));   % PSNR of recovery image